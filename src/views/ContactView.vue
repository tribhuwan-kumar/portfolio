<template>
    <section class="md:min-h-[90vh] min-h-screen w-full flex flex-col mt-48 mb-[-150px] lg:mb-auto lg:mt-[-50px]" ref="contactSection" id="contact-section">
        ...
        <div :class="['flex flex-col mt-10 transition-all motion-reduce:transition-none duration-500 delay-300', visible ? 'translate-y-0 opacity-1 blur-0' : 'translate-y-4 opacity-0 blur-sm']">
            ...
            <div v-else class="space-y-8 mx-auto mt-12 lg:mt-15">
                <h1 class="text-2xl lg:text-3xl font-extrabold text-center">Get in Touch</h1>
                ...
                <div class="flex flex-col sm:flex-row items-center justify-center space-y-5 sm:space-x-5">
                    <template v-if="content.externalLink.link.email">
                        <a :href="'mailto:'+content.externalLink.link.email">
                            <button class="py-2 px-6 bg-transparent border border-button-color/50 rounded-md shadow-sm shadow-button-color/20 hover:text-link-color active:text-white">Send an Email</button>
                        </a>
                    </template>
                    <template v-else>
                        <a :href="content.externalLink.link.other" target="_blank">
                            <button class="py-2 px-6 bg-transparent border border-button-color/50 rounded-md shadow-sm shadow-button-color/20 hover:text-link-color active:text-white duration-300">Reach Out</button>
                        </a>
                    </template>
                </div>
            </div>
